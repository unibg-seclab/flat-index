...
parser = argparse.ArgumentParser(
    description='Visualize performance evaluations by serialization format, '
                'compression algorithm and bandwidth.'
)
...
# Group performance evaluation files by compression and serialization
# combination
by_compression = defaultdict(list)
for filename in os.listdir(directory):
    match = re.search(r"-serialization=(\w+)-compression=(\w+)-", filename)
    if match:
        serialization, compression = match.groups()
        by_compression[(serialization, compression)].append(filename)

total_curves = []
labels = []
for serialization, compression in sorted(by_compression.keys()):
    print(f"[*] serialization={serialization}, compression={compression} ")
    total_time = {}
    filenames = by_compression[(serialization, compression)]
    ...
    labels.append(f"S={serialization}, C={compression}")

total_worsening_curves = compute_worsening_curves(total_curves)

print("[*] Visualize performance overhead")
plot(baseline_curve, total_curves, "overall-time-by-compression-and-bandwidth.pdf")
baseline_worsening_curve = [(k, 1, float("NaN")) for k in sorted(baseline_time.keys())]
plot(baseline_worsening_curve, total_worsening_curves, "overall-overhead-by-compression-and-bandwidth.pdf", worsening=True)
